...
let path3 (P (coord, dir)) ms =
    let rec aux p ms c =
        match ms with
        | x :: xs -> 
            match step p x, x with
            | P (coord, dir), Forward _ -> aux (P (coord, dir)) xs (fun a -> c (coord::a))
            | pos, _ -> aux pos xs c
        | [] -> c []

    aux (P (coord, dir)) ms (fun a -> coord :: a)
...
//i would assume barbaz is slower, as it has to make the function baz every time it recurses

//2.5
let bazSeq = 
    Seq.initInfinite (fun a -> baz a)
...
let nextElement (el: element) = 
    ...
    aux el [] el.Head |> 
        List.fold (fun acc a -> 
            [for c in a.Length.ToString() -> string c] @ string a.[0] :: acc
        ) []
